...
func UpdateService(token string, request shared.UpdateRequest) (int,string){
	...
	log.Println("Cachet Token Valid")
	...
	log.Println("Retrieved components from cachet")

	body, _ := ioutil.ReadAll(resp.Body)
	_ = resp.Body.Close()
	_ = json.Unmarshal(body, &cachetComponents)

	log.Println("Grafana Struct Data")
	log.Println(request)
	...
	for key, value := range request.Tags {
		log.Println("I am inside update service")
		var concernedServices []int
		log.Println("Component Data")
		log.Println(cachetComponents.Data)
		for _, data := range cachetComponents.Data {
			for _, data2 := range data.Tags {
				if key == data2 {
					log.Println("component match found")
					log.Println(key)
		...
		for _, c := range concernedServices {
			log.Println("Updating component in CachetHQ")
